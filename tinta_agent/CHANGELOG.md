...
## 2026.9.2
- **Fix (важно): агент больше не завершается при ожидаемом 403 во время enrollment.** Добавлен controlled exponential backoff (капped на 120с) для 403/429/5xx/сетевых ошибок, предотвращающий restart loop и избыточные повторные запросы к backend; только 404/410 (токен действительно недействителен) завершают процесс сразу. Логика вынесена в `src/enrollment.ts` для модульного тестирования (`agent.ts` сам ничего не экспортирует).
