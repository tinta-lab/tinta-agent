...
## 2026.9.2
- **Fix (важно): агент больше не аварийно завершается при ожидаемом 403 "согласие ещё не дано" от install-эндпоинта.** Реальный инцидент 2026-09-21 (V&T home / hub-nx5g9c): Supervisor перезапускал упавший контейнер почти мгновенно (`startup: services`, `boot: auto`), что превращалось в цикл перезапусков, каждый из которых расходовал одну из 10 попыток backend-лимита на 15 минут — пока лимит не исчерпывался и следующий (уже легитимный) запрос браузера тоже не получал 429. Теперь 403/429/5xx/сетевые ошибки обрабатываются повторными попытками с экспоненциальной задержкой (капped на 120с, что держит устойчивую частоту ниже лимита backend с запасом); только 404/410 (токен действительно мёртв) завершают процесс сразу. Логика вынесена в `src/enrollment.ts` для модульного тестирования (`agent.ts` сам ничего не экспортирует).
